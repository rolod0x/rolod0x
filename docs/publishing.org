...
In the short term, the main priorities are [full Firefox support][firefox], [integration with Rotki][rotki], [adding support for dividing the address book into multiple sections][sections], and [automatic import of address book entries from URLs][URL-import] specified by the user, and tackling a few rough edges.
